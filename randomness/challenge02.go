...
func main() {
    ...
    fmt.Println("How many tries would you like? ")
    
    // we declare our try var
    var tryTimes int
    fmt.Scan(&tryTimes)
    if tryTimes > 5 {
        fmt.Println("\n\n Whoah big guy, with that many tries\n let's take that number between 0 and 100!!\n\n")
    ...
    // place to hold our secret number
    ...
    // create a for loop that begins at 1 and loops to the tryTimes int
    for try := 1; try <= tryTimes; try++ {
        ...
        if try == tryTimes {
            // if the number of tries is equal to tryTimes int, print game over and also the correct number
